...
def _bn_function_factory(norm, lrelu, conv):
    def bn_function(*inputs):
        concated_features = torch.cat(inputs, 1)
        bottleneck_output = conv(lrelu(norm(concated_features)))
...
class _DenseLayer(nn.Sequential):
    ...
    def forward(self, *prev_features):
        bn_function = _bn_function_factory(self.norm1, self.lrelu1, self.conv1)
        if self.memory_efficient and any(prev_feature.requires_grad for prev_feature in prev_features):
            bottleneck_output = cp.checkpoint(bn_function, *prev_features)
        else:
            bottleneck_output = bn_function(*prev_features)
        new_features = self.conv2(self.lrelu2(self.norm2(bottleneck_output)))
        if self.drop_rate > 0:
